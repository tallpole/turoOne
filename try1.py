...
# give a specific folder name for each csv file with 
# a number which shows the difference to others
StatsNum = 0
def newFileName():
    global StatsNum
    newName = 'stats'+ str(StatsNum) + '.csv'
    StatsNum +=1
    return newName

# give the trip and add each trip to a accumulated one

tripAddedSum = []

def tripsAdd(tripAddS):
    global tripAddedSum
    tripAddedSum = tripAddedSum + tripAddS
    return tripAddedSum
...
# crawl1 is the function which help to write trips in each page
    def crawl1(self, outfile):
        self.login()
        time.sleep(SLEEP_SECONDS)
        trips = self.get_trips()
        # self.write_to_file(trips, outfile)
    ...
    def write_to_file(self, rows, out):
        ...
        with open(out, 'w') as f:
            w = csv.DictWriter(f, delimiter=',',
                fieldnames=['url_snippet','carnameModel','pickup', 'dropoff', 'cost', 'reimbursement_mileage', 'reimbursement_tolls', 'earnings']
                )
            w.writeheader()
            w.writerows(rows)
    ...
    def get_trip(self, reservation_url_snippet):
        ...
        try:
            nameModel = self.driver.find_element_by_class_name('name-and-model')
        except Exception:
            nameModel = ""
        ...
        try:
            ...
            return {
                ...
                'cost': None,
                'earnings': None,
                'reimbursement_tolls': None,
                'reimbursement_mileage': None,
            }
    ...
    def get_trips(self, page_slug = None):
        ...
        print trip_details
# I want to write to the file from here, just when get the trip here


        # outfile_tep = newFileName()
        outfile_tep = "stats.csv"


        if len(sys.argv) > 1:
            outfile_tep = sys.argv[1]
        self.write_to_file(tripsAdd(trip_details), outfile_tep)
